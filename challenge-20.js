(function (win, doc) {
    ...
    */
    ...
    // let username = prompt('Qual o seu nome?') || 'Desconhecido'; //Curto Circuito || é "ou"
    ...
    */
    $button.addEventListener('click', function (event) {
        ...
        if (isValidName($inputUsername.value) && isValidEmail($inputEmail.value) && isValidMessage($message.value)) {
            return confirm('Tem certeza que quer enviar?') ? alert('Enviado com sucesso') : alert('Não enviado')
        }

    }, false)

    function isValidName(name) {
        return !!name ? true : alert('Preencha o nome de Usuário');
    }

    function isValidMessage(message) {
        return !!message ? true : alert('Preencha a mensagem');
    }
    ...
    function isValidEmail(email) {
        ...
        return regex.test(email) ? true : alert('Preencha um e-mail válido');
        /* Descritivo da função abaixo. Subi ela para deixar mais claro no arquivo
